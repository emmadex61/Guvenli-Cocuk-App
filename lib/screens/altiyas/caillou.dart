...
class caillou extends StatefulWidget {
  const caillou({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _caillouState();
}

class _caillouState extends State {
  get style => null;
  List<Video> videolar = [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          centerTitle: true,
          title: Text("Caillou Bölümler",style: TextStyle(
          color: Colors.white,
              ...
              itemBuilder: (_, index) {
                ...
                if (model.yasGrubu != null && model.yasGrubu!.contains(6)) {
                  if (model.cartoon != null &&
                      model.cartoon.contains('caillou')) {
                    return InkWell(
                      onTap: () {
                        Navigator.push(
                          context,
