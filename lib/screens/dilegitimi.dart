...
class dilegitimi extends StatefulWidget {
  const dilegitimi({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _dilegitimiState();
}

class _dilegitimiState extends State {
  get style => null;
  List<Video> videolar = [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          centerTitle: true,
          title: Text("Dil Eğitimi",style:TextStyle(color: Colors.white,
            ...
            ListView.builder(
              ...
              itemBuilder: (_, index) {
                ...
                if (model.yasGrubu != null && model.yasGrubu!.contains(13)) {
                  if (model.cartoon != null && model.cartoon.contains('dil')) {
                    return InkWell(
                      onTap: () {
                        Navigator.push(
                          context,
