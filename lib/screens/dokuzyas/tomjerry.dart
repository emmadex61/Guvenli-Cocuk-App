...
class tomjerry extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => _tomjerryState();
}

class _tomjerryState extends State {
  get style => null;
  List<Video> videolar = [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          centerTitle: true,
          title: Text("Tom ve Jerry Bölümler",style: TextStyle(
          color: Colors.white,
              ...
              itemBuilder: (_, index) {
                ...
                if (model.yasGrubu != null && model.yasGrubu!.contains(9)) {
                  if (model.cartoon != null &&
                      model.cartoon.contains('tomjerry')) {
                    return InkWell(
                      onTap: () {
                        Navigator.push(
                          context,
