...
class gumball extends StatefulWidget {
  const gumball({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _gumballState();
}

class _gumballState extends State {
  get style => null;
  List<Video> videolar = [];
              ...
              itemBuilder: (_, index) {
                ...
                if (model.yasGrubu != null && model.yasGrubu!.contains(13)) {
                  if (model.cartoon != null &&
                      model.cartoon.contains('gumball')) {
                    return InkWell(
                      onTap: () {
                        Navigator.push(
                          context,
