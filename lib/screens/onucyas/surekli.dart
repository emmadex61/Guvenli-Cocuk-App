...
class surekli extends StatefulWidget {
  const surekli({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _surekliState();
}

class _surekliState extends State {
  get style => null;
  List<Video> videolar = [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          centerTitle: true,
          title: Text("Sürekli Dizi Bölümler",style: TextStyle(
          color: Colors.white,
              ...
              itemBuilder: (_, index) {
                ...
                if (model.yasGrubu != null && model.yasGrubu!.contains(13)) {
                  if (model.cartoon != null &&
                      model.cartoon.contains('surekli')) {
                    return InkWell(
                      onTap: () {
                        Navigator.push(
                          context,
